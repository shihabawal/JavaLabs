...
class GameFrame extends JFrame implements ActionListener, Observer, Runnable {
		...
		while (true) {
			if (counter > 0) {
				...
				if (counter == 0) {
					...
					ourDriver = new driver("/dev/ttyUSB0");
					ourDriver.openPort();
					...
					if (ourDriver.isStopped()) {
						...
						ourDriver.getvalues();
						l2.setText("Distance is " + ourDriver.getDistance()
								+ " cm");
						ourDriver.closePort();
